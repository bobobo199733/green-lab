/* CDNURL/wp-content/themes/JointsWP-CSS-master/assets/scripts/anonshort-script-v3.js */
try{
(function(){function _escapeHtml(unsafe){return unsafe.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}
function _sanitizeInput(input){return jQuery('<div />').text(input).html()}
function _escapeHtmlAndSanitize(input){return _escapeHtml(_sanitizeInput(input))}
...
function shortenSuccess(res){var hashArray=[],shortLink=res.data.short_url;jQuery('#shortened_url').val(shortLink);jQuery('#shortened_url').trigger('change');jQuery('#unAuthShortenForm').hide();jQuery('#ShortenedForm').show();if(localStorage.getObj('hashes')!==null){hashArray=localStorage.getObj('hashes');hashArray.unshift(res.data);localStorage.setObj('hashes',hashArray)}else{hashArray.push(res.data);localStorage.setObj('hashes',hashArray)}
if(jQuery('.error-list').length>0){jQuery('.error-list').fadeOut('slow',function(){jQuery('.error-list').remove()})}
checkHistory()}
jQuery('#ShortenedForm').on('change keyup',function(){bitlink=_sanitizeInput(jQuery('#shortened_url').val());jQuery('#ShortenedForm').hide();jQuery('shortened_url').val('').trigger('change');jQuery('#shorten_url').val(bitlink).trigger('change');jQuery('#unAuthShortenForm').show();jQuery('#shorten_url').focus()});function checkHistory(){var hashArray=[];jQuery('#most_recent_links').empty();if(localStorage.getObj('hashes')!==null){hashArray=localStorage.getObj('hashes');var arrayLength=hashArray.length;if(arrayLength>=3){arrayLength=3}
for(var i=0;i<arrayLength;i++){raw=hashArray[i];longLink=hashArray[i].long_url;shortLink=hashArray[i].short_url;row='<li class="link"><span class="long-link">'+_escapeHtmlAndSanitize(longLink)+'</span><span><span class="short-link"><a href="'+_escapeHtmlAndSanitize(shortLink)+'" target="_blank" title="Shortened URL for '+_escapeHtmlAndSanitize(longLink)+'">'+_escapeHtmlAndSanitize(shortLink)+'</a></span><span class="copy"><button class="button button-secondary">Copy</button></span></span></li>';jQuery('#most_recent_links').append(row)}
jQuery('#shorten_actions').fadeIn();jQuery('.promo-customize').clone().insertAfter('#most_recent_links li:eq(0)').fadeIn()}}
var senderroraway;function shortenError(res){message=res.message;clearTimeout(senderroraway);if(message==undefined){message="Please check your link and try again."}
row='<ul class="error-list" style="display:none" role="alert"><li class="error text-center align-center"><span>'+_sanitizeInput(message)+'</span></li></ul>';if(jQuery('.error-list').length>0){jQuery('.error-list').removeClass('shorten-error').addClass('shorten-error')}else{jQuery('#shorten_actions').prepend(row);jQuery('.error-list').show();jQuery('#shorten_actions').fadeIn()}
jQuery('.error-list').show();senderroraway=setTimeout(function(){jQuery('.error-list').fadeOut(300,function(){jQuery('.error-list').remove()})},2500)}
function historySuccess(res){}
function historyError(res){}
jQuery('#unAuthShortenForm').on('submit',function(e){e.preventDefault();longUrl=jQuery('#shorten_url').val();BITLY.shorten(_sanitizeInput(longUrl),shortenSuccess,shortenError)});jQuery('#ShortenedForm').on('submit',function(e){e.preventDefault();var buttontimer;jQuery(this).find('.button').addClass('copied');clearTimeout(buttontimer);buttontimer=setTimeout(function(){jQuery('#ShortenedForm').find('.button').removeClass('copied')},1000);var element=jQuery('#shortened_url');element.select();document.execCommand("copy")});jQuery(window).load(function(){checkHistory()});jQuery(document).on('click','.copy .button',function(e){e.preventDefault();var buttontimer;jQuery(this).addClass('copied');clearTimeout(buttontimer);buttontimer=setTimeout(function(){jQuery('.copy .button').removeClass('copied')},1000);var element=jQuery(this).parents('li').find('.short-link a');var $temp=jQuery("<input class='temp-field'>");jQuery(this).after($temp);$temp.val(jQuery(element).text()).select();document.execCommand("copy");$temp.remove();jQuery(this).focus()})})();

}
catch(e){console.error("An error has occurred: "+e.stack);}
/* CDNURL/wp-content/themes/JointsWP-CSS-master/assets/scripts/marketo.js */
try{
jQuery(document).ready(function($){$('body').append();$('.bitly-form').each(function(){var tyStor='form-thank-you';var launch_form='#form_open';var lightbox;var marketo_form_id=$(this).attr('data-formid');var business_email_only=$(this).attr('data-business-emails-only');var thank_you_url=$(this).attr('data-thank-you-url');MktoForms2.loadForm("//app-ab01.marketo.com","754-KBJ-733",marketo_form_id,function(form){$('.mktoButtonWrap').removeClass('mktoSimple');$('.mktoButton').addClass('button button-primary button-wide').removeClass('mktoButton');if($('.mktoForm .mktoTemplateBox').length>0){console.log('old pal');$('.mktoForm .form-intro-title, .mktoForm .form-intro-subtitle').fadeOut()}
if(business_email_only=='true'){form.onValidate(function(){var email=form.vals().Email;if(email){if(!isEmailGood(email)){form.submitable(!1);var emailElem=form.getFormElem().find("#Email");form.showErrorMessage("Please provide a business email",emailElem);var containertop=emailElem.offset().top-90;$('html, body').animate({scrollTop:containertop},500)}else{form.submitable(!0)}}})}
form.onSuccess(function(vals,tyURL){console.log('sent');var formEl=form.getFormElem()[0];if(thank_you_url!=''){window.location=thank_you_url}else{$('.bitly-form > form').hide();$('.form-thank-you').fadeIn();var containertop=$(".entry-content").offset().top-80;$('html, body').animate({scrollTop:containertop},500);if($('#_download').length>0){window.open($("#_download").attr('href'))}
if($('.resource-box .responsive-embed').length>0){formEl.parentNode.removeChild(formEl);video=$('.resource-box .responsive-embed').html();$('.entry-content .resource-thumb').html(video)}}
return!1})});$container=$(this);if($(this).hasClass('modal-form')){console.log('has class')
$(launch_form+'_'+marketo_form_id).on("click",function(e){MktoForms2.whenReady(function(form){var formEl=form.getFormElem()[0],tyEl=formEl.querySelector(tyStor);lightbox=MktoForms2.lightbox(form,{onSuccess:function(vals,tyURL){formEl.parentNode.appendChild(tyEl);formEl.parentNode.removeChild(formEl);tyEl.style.display="block";if($('#_download').length>0){console.log('download');window.open($("#_download").attr('href'))}
return!1}});lightbox.show()})})}})
var invalidDomains=["@aol.","@att.","@comcast.","@facebook.","@gmail.","@gmx.","@googlemail.","@google.","@hotmail.","@mac.","@me.","@mail.","@msn.","@live.","@sbcglobal.","@verizon.","@yahoo.","@email.com","@fastmail.","@games.","@hush.","@hushmail.","@icloud.","@iname.","@inbox.","@lavabit.","@love.","@outlook.","@pobox.","@protonmail.","@rocketmail.","@safe-mail.","@wow.","@ygm.","@ymail.","@zoho.","@yandex.","@bellsouth.","@charter.","@cox.","@earthlink.","@juno."];function isEmailGood(email){for(var i=0;i<invalidDomains.length;i++){var domain=invalidDomains[i];if(email.indexOf(domain)!=-1){return!1}}
return!0}});

}
catch(e){console.error("An error has occurred: "+e.stack);}
/* CDNURL/wp-content/themes/JointsWP-CSS-master/assets/scripts/scripts.js */
try{
jQuery(document).foundation();jQuery(document).ready(function(){jQuery('.accordion p:empty, .orbit p:empty').remove();jQuery('iframe[src*="youtube.com"], iframe[src*="vimeo.com"]').each(function(){if(jQuery(this).innerWidth()/jQuery(this).innerHeight()>1.5){jQuery(this).wrap("<div class='widescreen responsive-embed'/>")}else{jQuery(this).wrap("<div class='responsive-embed'/>")}})});jQuery(document).ready(function($){$('#main-nav a[href="#"]').on('click',function(e){e.preventDefault()});$('#sidemenu a[href^="#"],#sidemenu-mobile a[href^="#"], a.smooth').on('click',function(e){e.preventDefault();element=$(this).attr('href');var width=$(window).width();if(width<1023){$('.off-canvas').removeClass('is-transition-push is-open');$('.js-off-canvas-overlay').removeClass('is-visible is-closable')}
$('html, body').animate({scrollTop:$(element).offset().top-120},1000)})
$('section.accordion-block.Horizontal .tabs-title').mouseenter(function(event){$('section.accordion-block.Horizontal .tabs-title').removeClass('is-active');$('.tabs-panel').removeClass('is-active').attr('aria-hidden','true');$(this).addClass('is-active');panel=$(this).attr('data-toggle');$('.tabs-panel').attr('aria-hidden','true');$(panel).addClass('is-active').attr('aria-hidden','false')});$(' section.accordion-block.Horizontal .tabs-title a').on('click',function(){window.location=$(this).attr('href')});if($('.language--select').length>0){$('.language--select').on('change',function(){url=$(this).val();window.location=url})}
if($('section.comparison-tables').length>0){var plans=[];$('.plan-names td').each(function(index,item){planname=$(this).html();plans.push(planname)});$('section.comparison-tables tr').each(function(index,item){$(this).find('td').each(function(indextd,item){if(plans[indextd]!='–'){$(this).attr('data-plan',plans[indextd])}})});$('.comparison-tables .table tr.features').on('click',function(){$(this).toggleClass('is-active')})}
if($('section.gallery-block.careers-grid').length>0){$('.gallery-row-container').attr('id','page-careers-benefits-photo-grid');$('.gallery-row-container').addClass('show-for-large');$('.gallery-row-container .gallery-image').each(function(count,item){var chr=String.fromCharCode(97+count);$(this).wrap('<div class="page-careers-benefits-photo-grid-cell page-careers-benefits-photo-grid-cell-'+chr+' inactive"></div>')
caption=$(this).attr('alt');$(this).after('<div class="benefit-caption"><div class="benefit-caption-inner">'+caption+'</div></div>')});$('section.gallery-block').append('<div class="slide-orbit hide-for-large"></div>')
$('.slide-orbit').append('<div class="slide-orbit-container owl-carousel">');$('.page-careers-benefits-photo-grid-cell').each(function(){$(this).clone().attr('class','').addClass('item-orbit-slide').appendTo('.slide-orbit-container')});$('.item-orbit-slide').each(function(){$(this).find('img').wrap('<figure class="item-orbit-figure">');$(this).find('.benefit-caption').wrap('<figcaption class="item-orbit-caption">')});var owl=$('.slide-orbit-container').owlCarousel({margin:10,loop:!0,dots:!1,center:!1,nav:!1,autoHeight:!0,autoplaySpeed:1000,navSpeed:1000,dotsSpeed:1000,autoWidth:!1,navText:['<span class="mv-left"><</span> ',' <span class="mv-right">></span>'],items:1,responsiveClass:!0,autoplay:!0,autoplayTimeout:5000,})}
if($('section.testimonial-block ').length>0){if($('section.testimonial-block.v2').length>0){dots=!0}else{dots=!1}
var testimonials=$('.testimonial-block .testimonials').owlCarousel({margin:40,loop:!0,dots:dots,center:!1,nav:!0,autoHeight:!0,autoplaySpeed:1000,navSpeed:1000,dotsSpeed:1000,autoWidth:!1,navText:['<span class="mv-left"></span> ',' <span class="mv-right"></span>'],items:1,responsiveClass:!0,autoplay:!1,autoplayTimeout:7000,responsive:{0:{nav:!1},1023:{nav:!0}}})
if($('section.testimonial-block.v2').length>0){items=$('.owl-dots .owl-dot ').length;margins=items*20;$('.owl-prev').css('margin-right',margins);$('.owl-next').css('margin-left',margins)}}
if($('section.gallery-block.staff-animation').length>0){var els=$('.staff-animation .gallery-image');var half=$('.staff-animation .gallery-image').length/2;els.slice(0,half).wrapAll('<div class="gallery-row"></div>');$('.gallery-row-container > img').wrapAll('<div class="gallery-row"></div>');$('.gallery-row').css('transform','translateX(0px)')
clonedimg=0;var animateSlider=function(slider,start){$('.gallery-row').each(function(index,item){if(index==0){$(this).css('transform','translateX(-'+start+'px)')}else{$(this).css('transform','translateX('+start+'px)')}});animationwidth=$('.gallery-row-container').width()+150;if((start%150)==0&&start>animationwidth){$('.gallery-row-container > .gallery-row:first-child > img').eq(clonedimg).clone().appendTo('.gallery-row-container > .gallery-row:first-child').addClass('cloned');$('.gallery-row-container > .gallery-row:last-child > img').eq(clonedimg).clone().appendTo('.gallery-row-container > .gallery-row:last-child').addClass('cloned');clonedimg++}
start++;window.requestAnimationFrame(function(){animateSlider(slider,start)})}
jQuery(window).load(function(){var slider=document.querySelector('.gallery-row-container');animateSlider(slider,1)})}
$('.top-bar-right .menu-icon').on('click',function(){screensize=window.innerHeight;itemheight=$('.off-canvas .top-bar').outerHeight()+$('.off-canvas > .accordion-menu').outerHeight()+$('.off-canvas > .nav-wrap-mobile').outerHeight();padding=screensize-itemheight;if(padding>0){$('.off-canvas > .nav-wrap-mobile').css('margin-top',padding+'px')}})
$('.team-block .accordion-content a').attr('target','_blank');$('.gallery-icon img').attr('width','');$('.gallery-icon img').attr('height','');window.onload=function(){$('input[type="checkbox"][data-toggle-all]').prop('checked',!1);$('.is-year').removeClass('is-year')};if($('.pricing-tables').length>0){$(window).bind("pageshow",function(event){if(event.originalEvent.persisted||performance.getEntriesByType("navigation")[0].type==='back_forward'){$('input[type="checkbox"][data-toggle-all]').prop('checked',!1);$('.is-year').removeClass('is-year')}else{}})}
$('.pricing-tables [data-toggle-all]').click(function(){var $basicSignupLink1=$('#basic-plan-btn-top');var billing1=this.checked?'month':'annual';$basicSignupLink1.prop({href:$basicSignupLink1.prop('href').replace(/month|annual/,billing1)});var $premiumSignupLink1=$('#premium-plan-btn-top');var billing1=this.checked?'month':'annual';$premiumSignupLink1.prop({href:$premiumSignupLink1.prop('href').replace(/month|annual/,billing1)})});$('.comparison-tables [data-toggle-all]').click(function(){var $basicSignupLink2=$('#basic-plan-column-link-top');var billing2=this.checked?'month':'annual';$basicSignupLink2.prop({href:$basicSignupLink2.prop('href').replace(/month|annual/,billing2)});var $basicSignupLink3=$('#basic-plan-column-link-bottom');var billing3=this.checked?'month':'annual';$basicSignupLink3.prop({href:$basicSignupLink3.prop('href').replace(/month|annual/,billing3)});var $premiumSignupLink2=$('#premium-plan-column-link-top');var billing2=this.checked?'month':'annual';$premiumSignupLink2.prop({href:$premiumSignupLink2.prop('href').replace(/month|annual/,billing2)});var $premiumSignupLink3=$('#premium-plan-column-link-bottom');var billing3=this.checked?'month':'annual';$premiumSignupLink3.prop({href:$premiumSignupLink3.prop('href').replace(/month|annual/,billing3)})});formstackcount=$('.ajax-me .fsForm').length;if(formstackcount>0){$('.ajax-me').after('<iframe name="formsubmit" id="formsubmit" style="width:0;height:0;opacity:0"></iframe')
$('.ajax-me').after('<style>.page .fsBody .fsForm .fsSubmit input.fsSubmitButton[disabled]{background-color:#666!important}</style')
$('.ajax-me .fsForm').attr('target','formsubmit');var message=$('.ajax-me').attr('data-message');$(".ajax-me .fsForm").submit(function(event){errors=$('.ajax-me .fsForm .fsValidationError').length;if(errors===0){$('#formsubmit').on('load',function(){$('.fsSubmitButton').attr('value','Submitted');thanks=$('.ajax-me .fsForm .thanks').length;if(thanks===0){$('.fsSubmitButton').after('<br><br><strong class="thanks">'+message+'</strong>')}
$('.fsSubmitButton').prop("disabled",!0)})}})}
jQuery(document).on('mousedown','.card-style-trust .card-cta a,.card-style-trust .card',function(e){var buttontimer;jQuery(this).parents('.cell').addClass('click');clearTimeout(buttontimer);buttontimer=setTimeout(function(){jQuery('.cell').removeClass('click')},500)});jQuery('section.card-style-trust .grid-x .cell .card a').not('section.card-with-list .card a').on('click',function(e){e.preventDefault()})
jQuery('section.card-style-trust .grid-x .cell .card').not('section.card-with-list .card').on('click',function(e){theurl=jQuery(this).find('.card-cta a').attr('href');window.location=theurl});$('a[data-open="qr-modal"]').on('click',function(e){e.preventDefault();$('#qr-modal').addClass('open');setTimeout(function(){$('.progress').addClass('active')},1000);setTimeout(function(){url=$('.reveal a.qr-link').attr('href');window.location=url},3000)})
$('#unAuthShortenForm-preview').on('submit',function(e){e.preventDefault();link=$('#shorten_url_preview').val().replace('http://bit.ly/','').replace('http://bitly.com/','');contentcontainer='.bitlink--metadata-wrapper';if(location.hostname=="bitly.com"||location.hostname=="bitly.net"){proxyurl='http://bitly.com/'+link+'+'}else{proxyurl='/CDNURL/wp-content/themes/JointsWP-CSS-master/assets/scripts/preview.html?'+link+'+'};$('.bitlink-preview').load(proxyurl+" "+contentcontainer,function(data){})})});window.onload=function(){jQuery('body').addClass('page-loaded')};jQuery(window).load(function(){jQuery('body').addClass('page-loaded');if(jQuery('.reveal').length>0){jQuery(window).bind("pageshow",function(event){if(event.originalEvent.persisted){jQuery('.reveal').removeClass('open')}else{}})}
if(jQuery('body #banner-cookie').length>0){jQuery('header.header').addClass('gdrp-on');jQuery(document).on('click','#banner-cookie--button',function(e){jQuery('header.header').removeClass('gdrp-on')})}
jQuery('#stickytable #priceSwitch').on('change',function(){jQuery('.pricing-row').toggleClass('is-year')});if(jQuery('.menu-item a img,.tabs-title img, .button-primary img').length>0){jQuery('.menu-item a img,.tabs-title img, .button-primary img').each(function(){var $img=jQuery(this);var imgURL=$img.attr('src');jQuery.ajax({url:imgURL,dataType:'xml',cache:!0,method:'get',}).done(function(data){var $svg=jQuery(data).find('svg');$svg=$svg.removeAttr('xmlns:a');$img.replaceWith($svg)})})}});

}
catch(e){console.error("An error has occurred: "+e.stack);}
/* CDNURL/wp-includes/js/jquery/jquery.min.js */
try{
...
/* CDNURL/wp-includes/js/jquery/jquery-migrate.min.js */
try{
...
/* CDNURL/wp-content/plugins/wow-modal-windows-pro/public/assets/js/jquery.effects.min.js */
try{
...
/* CDNURL/wp-content/plugins/wow-modal-windows-pro/public/assets/js/jquery.modalWindow.min.js */
try{
...
try{
jQuery(function() {jQuery("#wow-modal-overlay-12").ModalWindow({"animation":["no","400","no","400"],"overlay":false,"action":["load","8"],"closeBtn":[false,"0"],"closeAction":[false,false],"cookie":[true,"3","wow-modal-id-12"],"triggers":["wow-modal-id-12","wow-modal-close-12","wow-button-close-12"],"modal_css":{"width":"550px","height":"auto","bottom":"3%","right":"2%","padding":"39px","border-width":0,"border-style":"none","border-color":"#383838","position":"fixed","border-radius":"4px","background-color":"#ffffff","box-shadow":"0 10px 12px 1px rgba(0,0,0,0.09)"},"content_css":{"font-family":"inherit","font-size":"16px"}}); });
}
catch(e){console.error("An error has occurred: "+e.stack);}
