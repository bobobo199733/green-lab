...
export const initFullstoryCallback = (() => {
...
export const makeFullStoryTracker = waitForConsent => {
...
export const TRACKING_ENDPOINT_STAGE = 'https://at.stage.jimdo.systems/rich';
export const TRACKING_ENDPOINT_PROD = 'https://at.prod.jimdo.systems/rich'; // Monotonically increasing ticker for sent messages. This allows us to detect if messages went missing or arrived out
// of order

let seq = 0; // A session ID. The purpose of this ID is to uniquely identify a session, within which the `seq` is supposed to be
...
export const TRACKING_SID = generateSid();
export const RichTracking = config => event => {
  ...
  if (config !== null && config !== void 0 && config.fullStoryConfig && !fullStoryTracker) {
    ...
    fullStoryTracker = makeFullStoryTracker(waitForConsent);
  }

  if (fullStoryTracker && !isFsTrackingBlacklistedItem(topic)) {
    fullStoryTracker('dwh', {
      topic,
      ...message,
      anonId: data.anonId,
      env: data.env
...
export function getRichTrackingEndpoint(hostname) {
