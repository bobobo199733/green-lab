...
var do_PrePlugins = function() {
    ...
    switch (s.server) {
        ...
        case "www.rakuten.co.jp":
            myEvent = "Others: www: "
            if (!s.pageName) {
                s.pageName = myEvent + myPath.split("/")[0];
            }
    ...
    try {
        ...
        if (!s.eVar33) {
            var _s_Channel = s.channel.replace(_delUAInfo, "");
            if (_s_Channel.indexOf("Cart") == 0) {
                _s_Channel = "Cart";
            }
            var _s_tmpChannel = s.getCk("s_prechannel").replace(_delUAInfo, "");
            if (!_s_tmpChannel) {
                s.setCk("s_prechannel", "NoneCookie");
            }
...
s.GenderAgeGet = function() {
    s.initMrcr();
    s.gaRetval = "100";
    if (s.s_mrcrArr[1] && s.s_mrcrArr[2] && s.s_mrcrArr[3]) {
        s.gaRetval = s.s_mrcrArr[1] + s.s_mrcrArr[2] + s.s_mrcrArr[3]
    }
    return s.gaRetval
};
...
s.initMrcr = function() {
    if (!s.s_mrcrArr) {
        ...
        if (s.s_mrcr) {
            for (s.s_mrcri = 0; s.s_mrcri < s.s_mrcr.length; s.s_mrcri++) {
                s.s_mrcrArr.push(s.s_mrcr.charAt(s.s_mrcri))
            }
        } else {
            s.s_mrcrArr.push("1");
            s.s_mrcrArr.push("1");
            s.s_mrcrArr.push("0");
            s.s_mrcrArr.push("0");
            for (s.s_mrcri = 4; s.s_mrcri < 200; s.s_mrcri++) {
                s.s_mrcrArr.push("0")
            }
            s.saveMrcr()
        }
...
s.saveMrcr = function() {
    ...
    for (s.s_mrcri = 4; s.s_mrcri < s.s_mrcrArr.length; s.s_mrcri++) {
        if (s.s_mrcri < 50) {
            s.s_mrcrBinWrite0 = s.s_mrcrBinWrite0 + s.s_mrcrArr[s.s_mrcri]
        } else if (s.s_mrcri >= 50 && s.s_mrcri < 100) {
            s.s_mrcrBinWrite1 = s.s_mrcrBinWrite1 + s.s_mrcrArr[s.s_mrcri]
        } else if (s.s_mrcri >= 100 && s.s_mrcri < 150) {
            s.s_mrcrBinWrite2 = s.s_mrcrBinWrite2 + s.s_mrcrArr[s.s_mrcri]
        } else {
            s.s_mrcrBinWrite3 = s.s_mrcrBinWrite3 + s.s_mrcrArr[s.s_mrcri]
        }
    }
...
s.userAgentManager = function() {
    ...
    if (s.sc_ua.indexOf("ipod") > -1) {
        ...
        s.sc_ua_ret = "BlackBerry"
    } else if (s.sc_ua.indexOf("windows ce") > -1) {
        s.sc_ua_ret = "Windows Mobile"
    } else if (s.sc_ua.indexOf("nintendo wii") > -1) {
        s.sc_ua_ret = "Nintendo Wii"
    } else if (s.sc_ua.indexOf("nintendo ds") > -1) {
        s.sc_ua_ret = "Nintendo DS"
