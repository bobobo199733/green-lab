...
/** THIS FILE ALSO CONTAINS A PATCH AT THE BOTTOM WHICH IS REQUIRED FOR THE 
 * DEPRECATED FEATURES OF AEM ADAPTIVE FORMS UNDERSCORE LIBRARY
 */
...
(function(_, window) {
    /**
     * forEach() thisArg PATCH for AAF
     *
     * In older version of lodash (v3.10.1), forEach() used to have a third parameter (_thisArg)
     * which has been removed from the later versions, however it's still being used by AAF.
     * This patch will add _thisArg again to forEach().
     */
    _.forEachOriginal = _.forEach;

    _.each = _.forEach = function(collection, iteratee, thisArg) {
        if (!thisArg) thisArg = window;
        return _.forEachOriginal(collection, _.bind(iteratee, thisArg));
    };

    /**
     * find() _thisArg PATCH for AAF
     *
     * In older version of lodash (v3.10.1), find() used to have a third parameter as "this" binding of predicate.
     * In the latest version, this has been chanegd to an optional index parameter, however _thisArg is still being used by AAF.
     * This patch will add _thisArg again to find().
     */
    _.findOriginal = _.find;

    _.find = function(collection, iteratee, param3) {

        var index = 0; // to support new lodash version _.find syntax
        var thisObj = param3 || window; // to support old lodash version _.find syntax

        var lastParam;

        // param3 can be index (a number) or "this" object, so have to check and set the value for lastParam
        lastParam = param3 ? (_.isNumber(param3) ? param3 : thisObj) : index;

        if (_.isNumber(lastParam)) {
            return _.findOriginal(collection, iteratee, lastParam);
        } else {
            return _.findOriginal(collection, _.bind(iteratee, lastParam), 0);
        }
    };

    /**
     * contains() PATCH for AAF
     *
     * contains is an underscore funtion, and is being referenced in AAF library scripts has been deprecated
     * in our 4.17.21 lodash version. So applying the below patch as an alias for it.
     */
    _.contains = function(collection, value, fromIndex) {
