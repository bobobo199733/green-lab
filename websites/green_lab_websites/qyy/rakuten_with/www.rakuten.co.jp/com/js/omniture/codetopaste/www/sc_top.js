...
(function(_W, _D, _N, _L) {
    ...
    SC.addJavaScript = function() {
        ...
        var ele1, ele2, ele3, ele4, ele5, customClick1, customClick2, customClick3, customClick4, customClick5;
        //SP and PC
        ...
        // SP only: searchCancel (DA-340)
        ele5 = _D.getElementById('searchCancel');
        if (ele5) {
            ele5.style.cursor = 'pointer';
            customClick5 = function() {
                _W.SC.sendSTL({
                    'prop18': 'searchCancel'
                }, 'searchCancel');
            };
            SC.addEvent(ele5, 'click', customClick5);
        }
    ...
    SC.addJquery = function() {
        try {
            ...
            // DA-1528: Popup tracking for policy announcement using 'data-track-popup' parameter
            SC.$ele16 = $('#sc-track-popup');
            if (SC.$ele16.length) {
                SC.$ele16.bind('click', function(event) {
                    var linkpopup = $(this).attr("data-track-popup");
                    if (linkpopup) {
                        _W.SC.sendScDataOnLoad({
                            'prop18': linkpopup
                        });
