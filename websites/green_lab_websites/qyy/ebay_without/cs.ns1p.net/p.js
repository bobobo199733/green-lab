! function(t) {
    ...
    s.m = t, s.c = e, s.d = function(t, e, i) {
        ...
        if (s.r(i), Object.defineProperty(i, "default", {
            ...
            for (var n in t) s.d(i, n, function(s) {
                return t[s]
            }.bind(null, n));
        return i
    }, s.n = function(t) {
    ...
    class r {
        constructor(t, s) {
            this.params = s, this.pulsarDomain = t, this.results = [], this.scheme = n(s.s), this.runs = s.N ? parseInt(s.N, 10) : 3, this.totalRuns = this.runs, this.defaultTimeout = 15e3, this.randString = this.randString.bind(this), this.randQueryString = this.randQueryString.bind(this), this.run = this.run.bind(this), this.abort = this.abort.bind(this)
        }
        randString(t) {
    ...
    class o extends r {
        ...
        async measure() {
            const {
                ...
                a: o,
                e: a
            } = this.params, h = setTimeout(this.abort, r || 3e3);
            return new Promise((r, u) => {
                if (!n || !this.scheme) return clearTimeout(h), r(null);
                const c = t || `${s.toLowerCase()}.${this.pulsarDomain}`,
                    l = `${e||"/a/r.js"}${this.randQueryString()}`,
                    d = `${this.scheme}//${c}${l}&j=${n}`;
                setTimeout(() => {
                    i.get(d).then(t => {
    ...
    var u = {
        parse: t => (t => {
            const s = parseInt(t.fetchStart, 10),
                e = h.map(e => {
    ...
    class c extends r {
        ...
        loadImg(t, s) {
            ...
            const i = (new Date).getTime(),
                ...
                }, this.params.t || this.defaultTimeout),
                r = (n, r) => () => {
        ...
        loadImgCallback(t, s, e, i) {
            ...
            return e = e || 500, !this.useLegacy && !n && this.tryXhr && !this.skipXhr && 1 == this.totalRuns - this.runs ? (this.skipXhr = !0, this.loadImg(s, i)) : this.aborted ? i(null) : void(this.useLegacy || void 0 === n ? i({
                status: parseInt(e),
                measurements: u.parse({
                    fetchStart: 0,
                    responseEnd: r - t
                })
            }) : i(Object.assign({}, {
                status: parseInt(e),
                measurements: u.parse(n)
            })))
        }
        getEntry(t) {
        ...
        serializeResponses() {
            ...
            return `${e},${t},${i||""}:${this.results.filter(t=>t).map(t=>{return`${s?"x":"i"}${a.base36(t.status||0)},${t.measurements}`}).join("|")}`
        }
    }
    const l = async t => new Promise(s => {
        setTimeout(s, t)
    });
    class d {
        constructor(t, s, e) {
            let i, n;
            this.appIds = t, this.tags = s, this.domain = e || "//s.ns1p.net/";
            const r = window.document.getElementById("ns1ps");
            n = (i = window.parent !== window && r && "script" === r.nodeName.toLowerCase() ? window.parent : window).document, this.w = i, this.d = n, this.jobtypes = {}, this.c = {}, this.running = 0, this.scheduler = "//s.ns1p.net/", this.beacon = "//b.ns1p.net/", this.updateEndpoint = "//c.ns1p.net/u.html", this.updateSSLEndpoint = "cs.ns1p.net/u.html", this.version = "1619458322", this.insertScript = this.insertScript.bind(this), this.sendBeacon = this.sendBeacon.bind(this), this.joinResults = this.joinResults.bind(this), this.forceReload = this.forceReload.bind(this), this.runJobs = this.runJobs.bind(this), this.runByScript = this.runByScript.bind(this), this.fetchScript = this.fetchScript.bind(this), this.fetchScript(`${this.d.location.protocol}${this.scheduler}?v=${this.version}&a=${this.appIds}`)
        }
        fetchScript(t) {
            i.get(t).then(t => {
        ...
        insertScript(t) {
            const s = this.d.createElement("script"),
                e = this.d.getElementsByTagName("script")[0];
        ...
        async runJobs() {
            ...
            let s = [],
                e = 0;
            for (const t of this.xact.j) {
                const i = "L" === t.J ? new c(this.domain, t) : new o(this.domain, t);
                s.push(i), "number" == typeof t.w && e < t.w && (e = t.w)
            }
            l(e);
            for (const e of s) t.push(await e.run());
            this.sendBeacon(t), 1 === this.xact.R && this.forceReload()
        }
        forceReload() {
            if (void 0 !== this.d.body && this.d.body) {
                const t = this.d.createElement("iframe"),
                    s = n();
    ...
    class p {
        ...
        getDomain() {
            return {
                NODE_ENV: "production",
                PROBABILITY: 1,
                SCHEDULER: "//s.ns1p.net/",
                BEACON: "//b.ns1p.net/",
                UPDATE: "//c.ns1p.net/u.html",
                ...
                VERSION: "1619458322",
                RPORT: void 0
            }.PULSAR_DOMAIN : window.location.href.includes("u.html") ? Array.from(document.querySelectorAll("script")).map(t => t.src).find(t => t.includes(this.id)).split("/p.js")[0] : [...document.querySelectorAll("script")].map(t => t.src).find(t => t.includes(__nspid)).split("//")[1].split("/p.js")[0].split(".").slice(-2).join(".")
        }
        execute() {
